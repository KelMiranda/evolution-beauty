...
function makeNext(impl: NextFn = async () => new Response('ok')): NextFn {
...
describe('Astro middleware — SPA redirect for non-API paths', () => {
  ...
  it('redirects a /cursos/9 request (no token) to the SPA with the hash added', async () => {
    const next = makeNext();
    const response = await onRequest(makeContext('http://localhost:4321/cursos/9'), next);
  ...
  it('redirects the root path / to the SPA root with the hash containing the path', async () => {
    const next = makeNext();
    const response = await onRequest(makeContext('http://localhost:4321/'), next);
  ...
  it('does NOT redirect /api paths and forwards them to next()', async () => {
    const next = makeNext();
    const response = await onRequest(makeContext('http://localhost:4321/api/courses'), next);
