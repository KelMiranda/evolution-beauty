...
test.describe('Full User Flow', () => {
  ...
  test('login flow and navigate to dashboard', async ({ page }) => {
    await page.getByLabel('Correo electrónico').fill('admin@acoes.local');
  ...
  test('dashboard view for admin', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /entrar/i }).click();
    await expect(page).toHaveURL(/#\/dashboard/);
    await expect(page.getByText(/dashboard/i).or(page.getByText(/panel/i)).first()).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Public Pages', () => {
  ...
  test('courses catalog loads without errors', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /formación de excelencia/i })).toBeVisible({ timeout: 10000 });
  });
